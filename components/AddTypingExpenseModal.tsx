...
import { TYPING_SERVICES_EXPENSE_SUGGESTIONS } from '../constants';
import { XIcon } from './common/Icons';

interface AddTypingExpenseModalProps {
  onClose: () => void;
}

const AddTypingExpenseModal: React.FC<AddTypingExpenseModalProps> = ({ onClose }) => {
      ...
      <div className="bg-dark-card rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-dark-text">Add Typing Cost</h2>
          ...
          <div>
            <label className="block text-sm font-medium text-dark-subtext">Cost Amount ({settings.currency})</label>
          ...
          <div>
            ...
            <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g., Adobe Photoshop Subscription" className="mt-1 w-full p-3 bg-gray-800 border border-gray-700 rounded-lg text-dark-text" required disabled={isSaving}/>
          </div>

          <div>
             <label className="block text-sm font-medium text-dark-subtext mb-2">Quick Add Description</label>
             <div className="flex flex-wrap gap-2">
                {TYPING_SERVICES_EXPENSE_SUGGESTIONS.map(tag => <button type="button" key={tag} onClick={() => setDescription(tag)} className="px-3 py-1 bg-gray-700 rounded-full text-xs text-dark-subtext hover:bg-gray-600" disabled={isSaving}>{tag}</button>)}
            </div>
          </div>
          ...
          <div className="pt-2">
            <button type="submit" disabled={isSaving} className="w-full px-6 py-3 font-bold text-white bg-orange-600 rounded-lg hover:bg-orange-700 disabled:opacity-50 disabled:cursor-wait">
...
export default AddTypingExpenseModal;